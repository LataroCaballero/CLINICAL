...
- ✓ Schema + acuse persistido de indicaciones: campo global set-once `Paciente.indicacionesLeidasAt`, `ConsentimientoFirmado.indicacionesLeidasAt` relajado a nullable (preserva timestamps forenses v1.12), aplicado con patrón pgBouncer sobre Supabase en vivo — v1.14 (INDIC-03)
- ✓ Firma del consentimiento desacoplada de indicaciones: `firmarConsentimiento` ya no requiere ni depende de `indicacionesLeidas` (removido del DTO); endpoint portal-scoped set-once de acuse de indicaciones — v1.14 (CONS-11)
- ✓ `computePasosCrm` deriva `indicacionesPreop` de `Paciente.indicacionesLeidasAt` (fuente primaria) con fallbacks legacy sin regresión; `getKanban` sin truncamiento `take:1` en `consentimientosFirmados` (pacientes multi-zona) + test de frontera y guard de source-shape en Jest — v1.14 (INDIC-04)
- ✓ Gate de firma en el portal: el botón exige abrir el PDF **y** tildar "Leí el consentimiento" (dos gates client-side independientes); sección de consentimiento sin contenido de indicaciones — v1.14 (CONS-09, CONS-10, CONS-12)
- ✓ Sección de indicaciones separada: `PortalIndicaciones.tsx` lista indicaciones preop por zona y dispara el acuse automáticamente al abrir el link (sin firma dibujada), como 5ª sección del acordeón del portal — v1.14 (INDIC-01, INDIC-02)
- ✓ Visibilidad staff + cierre W-1: el stepper del sheet muestra la fecha de lectura de indicaciones; el board CRM se mantiene fresco vía refetch on window focus (sin recarga manual) — v1.14 (INDIC-05, EMBUDO-06)
- ✓ Embudo CRM automático en los tres puntos del flujo: `create()` setea `etapaCRM=NUEVO_LEAD` + `flujo=null` (lead visible, no "Sin clasificar"); `crearTurnoCirugia` confirma sin depender del presupuesto aceptado, `crearTurno` sólo degrada etapas avanzadas si el turno es "Consulta" y `cancelarTurno` conserva CONFIRMADO + caliente exponiendo `requiereRecontacto` — v1.15 (EMBUDO-07, EMBUDO-08)
- ✓ Tratamiento en consultorio saca al paciente del kanban (`resolverTipoEntrada` server-side + branch TRATAMIENTO sobre `flujoActual=null|PENDIENTE` + limpieza de `etapaCRM`) y lo deja en la planilla con la fecha real del tratamiento, reusando el patrón v1.13 de ocultar sin enum ni columna nueva — v1.15 (EMBUDO-09)
- ✓ Pendiente por etapa en la card del kanban: badge "Dar turno" en `NUEVO_LEAD` y "Ser atendido" en `TURNO_AGENDADO`, en la zona de registro de contacto inferior — v1.15 (CONTACTO-03, CONTACTO-04)
...
- Nueva etapa/columna CRM "Tratamiento Realizado" — la salida a planilla se resuelve con flujo=TRATAMIENTO + ocultar del board (patrón v1.13); sin migrar el enum `EtapaCRM` (v1.15)
...
11/11 requisitos completados en 9 días (2026-07-31 → 2026-08-08). 4 fases (63–66), 9 planes, 20 tareas. Audit `tech_debt` (11/11 reqs, 4/4 fases verificadas, 9/9 seams WIRED, 1/1 flujo E2E new-lead→tratamiento, 0 blockers). El embudo del kanban pasó a reflejar solo el estado real del paciente: `create()` deja al lead nuevo en la columna "Nuevo Lead" (`etapaCRM=NUEVO_LEAD`, `flujo=null`) en vez de "Sin clasificar"; agendar la fecha de cirugía confirma al paciente sin depender del presupuesto aceptado; un turno nuevo ya no degrada etapas avanzadas salvo que sea de tipo "Consulta"; cancelar la cirugía conserva CONFIRMADO + interés caliente y expone `requiereRecontacto` en la lista de acción; y cargar una HC de "Tratamiento en consultorio" saca al paciente del board (flujo=TRATAMIENTO + limpieza de `etapaCRM`, patrón v1.13 sin enum ni columna nueva) dejándolo en la planilla con la fecha real del tratamiento. En el frontend, la card muestra el pendiente de su etapa ("Dar turno" / "Ser atendido") y la columna "Último tratamiento" dejó de perder información: un helper puro nuevo expone la lista completa por `GET /turnos/rango` y la celda la trunca por CSS con tooltip Radix. En paralelo, el tipo de turno se sincroniza solo con la plantilla de HC cargada encima (`resolverTipoTurnoSync`, escalera Consulta < Tratamiento < Pre-Quirúrgico con no-downgrade y sentinel de cirugía, dentro de la transacción de `crearEntrada` y con guard que lo vuelve no-op sin turno asociado o con `turnoId` stale), y la HC de la ficha del paciente renderiza por fin el detalle Pre-quirúrgico persistido, con el wizard "+ Nueva HC" como único camino de creación. **Cierre limpio:** 0 artefactos abiertos; se resolvieron los 3 ítems diferidos de v1.14 (`62-HUMAN-UAT`, `62-VERIFICATION` y el quick-task del dropdown de HC). **Deuda advisory aceptada:** D-06 gateado en el string `'Consulta'`; leads `flujo=null` no promovidos a CIRUGIA por la auto-clasificación genérica; `console.log` de error crudo en `pacientes.service.ts:76`; extractor de tratamientos sin guard de JSONB malformado; TOCTOU angosto e inalcanzable en el sync de turno; `esCirugia` no-op con el seed actual; `hasAny` de la preview omite `estudiosComplementarios`. Ver `.planning/milestones/v1.15-ROADMAP.md` para detalles.

## Shipped: v1.14 Portal — Firma Gated e Indicaciones Separadas ✅

10/10 requisitos completados en ~13 días (2026-07-08 → 2026-07-21). 2 fases (61–62), 8 planes, 14 tareas. Endureció el flujo legal del portal del paciente y separó las indicaciones del consentimiento. En backend: nuevo campo global set-once `Paciente.indicacionesLeidasAt` (acuse de lectura, INDIC-03) y `ConsentimientoFirmado.indicacionesLeidasAt` relajado a nullable preservando los timestamps forenses de v1.12, aplicado con el patrón pgBouncer sobre Supabase en vivo; `firmarConsentimiento` desacoplado de `indicacionesLeidas` (CONS-11, removido del DTO) + endpoint portal-scoped set-once de acuse; `computePasosCrm` deriva el paso `indicacionesPreop` del acuse del perfil con fallbacks legacy sin regresión, y `getKanban` deja de truncar `consentimientosFirmados` (fix multi-zona WR-01/SC#3) con test de frontera y guard de source-shape en Jest. En frontend: el botón de firma exige abrir el PDF **y** tildar "Leí el consentimiento" (dos gates client-side, CONS-09/10), la sección de consentimiento quedó limpia de indicaciones (CONS-12), y una sección nueva `PortalIndicaciones.tsx` lista las indicaciones por zona disparando el acuse automáticamente al abrir el link (INDIC-01/02); el staff ve la fecha de lectura en el stepper (INDIC-05) y el board CRM se mantiene fresco vía refetch on window focus, cerrando la deuda W-1 de v1.13 (EMBUDO-06). **Diferido al cierre (acknowledge & proceed):** 5 escenarios browser UAT de Phase 62 (`62-HUMAN-UAT` partial) + `62-VERIFICATION` (human_needed) — código verificado y wired — + quick-task carried `1-eliminar-dropdown-tipo-de-consulta-de-hc`. **Deuda de diseño aceptada tras UAT:** CR-03 — `indicacionesLeidasAt` es global/set-once pero el stepper lo muestra por zona (impreciso en multi-zona); WR-01 — acuse sin manejo de error visible. No se corrió `/gsd:audit-milestone`. Ver `.planning/milestones/v1.14-ROADMAP.md` para detalles.
...
**Estado actual (post-v1.15):** El embudo del kanban ya no depende de que alguien clasifique a mano. `pacientes.service.ts::create()` setea `etapaCRM=NUEVO_LEAD` + `flujo=null` incondicionalmente (el lead entra al board en su columna, apoyado en el filtro `OR:[{flujo:CIRUGIA},{flujo:null}]` de `getKanban`, sin ampliarlo ni exponer históricos PENDIENTE); `crearTurnoCirugia` mueve a CONFIRMADO sin presupuesto aceptado; `crearTurno` sólo degrada etapas avanzadas cuando el turno es de tipo "Consulta"; `cancelarTurno` conserva CONFIRMADO + interés caliente y `getListaAccion` expone `requiereRecontacto` derivado. Cargar una HC de "Tratamiento en consultorio" saca al paciente del board vía helpers puros nuevos en `historia-clinica.flujo.helpers.ts` — `resolverTipoEntrada` fuerza el `tipoEntrada` server-side desde `dto.tipo`, el branch TRATAMIENTO de `resolverNuevoFlujo` cubre `flujoActual=null` además de `PENDIENTE`, y `crearEntrada` limpia `etapaCRM=null` (espejo de `updateFlujo`) — quedando en la planilla con la fecha real del tratamiento. En el frontend, `PatientCard.tsx` muestra el pendiente de la etapa ("Dar turno" en NUEVO_LEAD, "Ser atendido" en TURNO_AGENDADO) y la columna "Último tratamiento" de `TratamientosTab` muestra la lista completa truncada por CSS con Radix Tooltip, alimentada por el helper puro `listarTratamientosDeContenido` y el campo `tratamientos: string[]` nuevo en `GET /turnos/rango` (el `ultimoTratamiento` colapsado sigue intacto). El tipo de turno se sincroniza solo con la plantilla de HC cargada encima: `resolverTipoTurnoSync` (escalera Consulta < Tratamiento < Pre-Quirúrgico vía rank map, no-downgrade, unmapped=0, sentinel de cirugía) corre dentro de la transacción de `crearEntrada` con `tx.turno.update`, sobre un `turnoCtx` pre-fetch extendido (patrón pgBouncer) y con el guard `if (dto.turnoId && turnoCtx)` que lo vuelve no-op sin turno asociado o con `turnoId` stale (evita el P2025 que abortaba el guardado completo de la HC). Y `HCEntryContent.tsx` suma la rama `pre_quirurgico` que renderiza el JSONB persistido en la ficha del paciente, con el wizard "+ Nueva HC" como único camino de creación tras eliminar el dropdown/form de texto libre muertos. 11/11 requisitos v1.15 completados en 9 días (4 fases, 9 planes, 20 tareas); audit `tech_debt` con 0 blockers y **0 artefactos abiertos al cierre** — los 3 ítems diferidos de v1.14 quedaron resueltos. **Deuda advisory registrada:** ver el bloque "Shipped: v1.15".

**Estado previo (post-v1.14):** El flujo legal del portal del paciente quedó endurecido y las indicaciones se separaron del consentimiento. Firmar un consentimiento de zona ahora exige dos gates client-side independientes — abrir/ver el PDF **y** tildar "Leí el consentimiento" — sin ninguna dependencia de las indicaciones (`firmarConsentimiento` desacoplado de `indicacionesLeidas`, removido del DTO). Las indicaciones preoperatorias viven en su propia sección del portal (`PortalIndicaciones.tsx`, 5ª del acordeón) que dispara el acuse de lectura automáticamente al abrir el link (sin firma dibujada); el acuse se persiste en un campo global set-once `Paciente.indicacionesLeidasAt` (con `ConsentimientoFirmado.indicacionesLeidasAt` relajado a nullable, preservando los timestamps forenses de v1.12). `computePasosCrm` deriva el paso `indicacionesPreop` de ese campo del perfil con las dos fuentes legacy como fallbacks sin regresión, y `getKanban` dejó de truncar `consentimientosFirmados` (fix multi-zona WR-01/SC#3) con test de frontera + guard de source-shape en Jest. El staff ve la fecha de lectura de indicaciones en el stepper del sheet y el board CRM se mantiene fresco vía refetch on window focus, cerrando la deuda W-1 de v1.13. 10/10 requisitos v1.14 completados en ~13 días (2 fases, 8 planes). **Diferido (acknowledge & proceed):** 5 escenarios browser UAT + `62-VERIFICATION` de Phase 62 (código verificado y wired) + quick-task carried; deuda de diseño aceptada CR-03 (stepper por zona vs campo global) y WR-01 (acuse sin onError). Ver STATE.md → Deferred Items.

**Estado previo (post-v1.13):** El tramo post-consulta del embudo CRM es ahora accionable y ordenado. `getKanban` (`pacientes.service.ts`) expone, vía el helper puro `computePasosCrm` (TDD), el estado de los 5 pasos (turno de cirugía con fecha/estado, entrada de HC relevante, presupuesto enviado/aceptado, consentimiento firmado, indicaciones preop) más el flag `todosCompletos` por paciente. La etapa "Cirugía Realizada" es una columna propia (reutiliza el enum `PROCEDIMIENTO_REALIZADO`), alimentada por un `@Cron` diario que mueve a los operados cuando su fecha de cirugía pasó; el guard forward-only se relaja para reactivar a `TURNO_AGENDADO` al crear un turno desde cualquier etapa. El board reordena las columnas (`Sin clasificar` al final, `Cirugía Realizada` tras Confirmado), oculta operados completos (tagueados para stats), marca en naranja los operados con pendientes y etiqueta las tarjetas de Confirmado ("Espera fecha"/"Cirugía programada"). El `EtapaStepper` del sheet lateral (`CardActionsSheet`) muestra cada paso verde/naranja y al click abre el modal que lo resuelve — `HCCreatorDialog`, `GenerarPresupuestoModal` prellenado desde catálogo, o `SurgeryAppointmentModal` (`POST /turnos/cirugia`) — con invalidación `['crm-kanban']` por acción; consentimiento/indicaciones son sub-indicadores de solo lectura (completan vía el portal v1.12, sin invalidación inmediata — W-1). `getKpis` calcula `cirugiasRealizadas`/`tratamientosRealizados` sobre registros reales (`Cirugia`/`HistoriaClinicaEntrada`) independientes de `etapaCRM`, expuestos como dos `KpiCard`s nuevos. 15/15 requisitos v1.13 completados en 3 días (4 fases, 8 planes); audit `tech_debt` con 0 blockers. **Diferido:** browser UAT de Phase 58 (5 escenarios) + display de KPI cards de Phase 60 (ver STATE.md → Deferred Items).
...
- `console.log('DTO RECIBIDO')` en `pacientes.service.ts:33` y `console.log('ERROR CAPTURADO EN CATCH:', error)` en `pacientes.service.ts:76` — exponen PII / error crudo en logs (v1.15 WR-03)
...
| Consentimiento/indicaciones como sub-indicadores read-only del stepper (sin quick-action ni invalidación) — v1.13 | Completan vía el portal v1.12 externo; agregar quick-action + invalidación quedó fuera de scope (D-04 Phase 59) | ✓ Resuelto en v1.14 — W-1 cerrada vía refetch on window focus en `['crm-kanban']` (EMBUDO-06); el board refleja consentimiento/indicaciones sin recarga manual |
| Campo de acuse de indicaciones global/set-once en `Paciente` (no per-zona en `ConsentimientoFirmado`) — v1.14 | Las indicaciones son un acto único de lectura del paciente, no per-zona; un campo de perfil es la fuente canónica y evita backfill | ⚠️ Revisit — deuda CR-03: el stepper muestra el acuse por zona pero el dato es global, impreciso en pacientes multi-zona (aceptada tras UAT) |
| `ConsentimientoFirmado.indicacionesLeidasAt` relajado a nullable vía DROP NOT NULL (no DROP COLUMN) — v1.14 | Preserva los timestamps forenses de consentimientos firmados en v1.12 mientras la fuente primaria pasa a `Paciente.indicacionesLeidasAt` (D-01) | ✓ Correcto — sin pérdida de registros; conserva la evidencia legal previa |
| `firmarConsentimiento` desacoplado de `indicacionesLeidas` (removido del DTO) + endpoint set-once de acuse — v1.14 | La firma y el acuse de indicaciones son actos legales independientes; acoplarlos bloqueaba firmas válidas (D-02/D-03/D-06/D-07) | ✓ Correcto — CONS-11; firma sin dependencia de indicaciones, acuse idempotente |
| Gate open-PDF + checkbox 100% client-side (sin tracking server-side de apertura) — v1.14 | La prueba legal es la firma dibujada + el checkbox de acuse (CONS-10); registrar "PDF abierto" en servidor no agrega valor probatorio (D-00) | ✓ Correcto — dos gates independientes habilitan la firma sin infra nueva |
| `getKanban` sin `take:1`/`orderBy` en `consentimientosFirmados` + guard de source-shape en Jest — v1.14 | El truncamiento rompía los fallbacks `.some()` legacy en pacientes multi-zona (WR-01/SC#3); el guard lee el source y falla si se reintroduce | ✓ Correcto — non-regresión garantizada por test de frontera + guard durable |
| Cierre W-1 vía refetch on window focus (no invalidación push ni realtime) — v1.14 | El staff vuelve a la pestaña tras enviar el link al paciente; refetch on focus alcanza sin websockets ni polling (fuera de scope) | ✓ Correcto — EMBUDO-06; board fresco sin recarga manual, `staleTime` bajado a 0 |
| `flujo=null` en `create()` en vez de ampliar el filtro de `getKanban` (D-03 opción 1) — v1.15 | Ampliar el filtro expondría en el board los históricos con `flujo=PENDIENTE`; escribir `flujo=null` en el alta deja pasar sólo a los leads nuevos por el `OR:[{flujo:CIRUGIA},{flujo:null}]` existente | ✓ Correcto — EMBUDO-07 sin backfill ni cambio de contrato del board |
| Salida a planilla vía `flujo=TRATAMIENTO` + ocultar del board (patrón v1.13), sin etapa nueva — v1.15 | Agregar "Tratamiento Realizado" al enum `EtapaCRM` obligaría a migrar y actualizar todos los consumidores (funnel, constantes frontend, STEPPER_CHAIN) | ✓ Correcto — EMBUDO-09 resuelto sin migración de enum |
| `resolverTipoEntrada` fuerza `tipoEntrada` server-side desde `dto.tipo` (D-08) — v1.15 | El cliente no debe poder desalinear el tipo de entrada del tipo de plantilla enviada; la clasificación de flujo depende de ese valor | ✓ Correcto — clasificación consistente e inmune al payload del cliente |
| Branch TRATAMIENTO de `resolverNuevoFlujo` cubre `flujoActual=null` además de `PENDIENTE` (D-09) + `crearEntrada` limpia `etapaCRM=null` (D-10) — v1.15 | Con EMBUDO-07 los leads nuevos entran con `flujo=null`, no `PENDIENTE`; sin cubrir ese caso el tratamiento no los sacaría del board. La limpieza de etapa espeja lo que ya hacía `updateFlujo` | ✓ Correcto — EMBUDO-09 funciona sobre leads creados post-v1.15 |
| Guard selectivo de degradación en `crearTurno` (sólo tipo "Consulta" resetea el ciclo) — v1.15 | Un turno de control o tratamiento no debería tirar atrás a un paciente ya CONFIRMADO; sólo una consulta nueva reinicia el ciclo del embudo (D-06) | ⚠️ Revisit — deuda WR-01: el gate depende del string mágico `tipoTurno.nombre === 'Consulta'`, frágil a rename/variante (sin flag en el schema) |
| Campo `tratamientos: string[]` nuevo en `/turnos/rango` en vez de cambiar `ultimoTratamiento` — v1.15 | Preserva el contrato colapsado existente para sus consumidores actuales y deja el display completo como decisión de frontend | ✓ Correcto — TRAT-07 sin regresión en consumidores del string colapsado |
| `resolverTipoTurnoSync` con rank map (`Record<string,number>`) en vez de cadena if/else — v1.15 | Expresa la escalera Consulta < Tratamiento < Pre-Quirúrgico de forma declarativa, con no-downgrade (D-01) y `unmapped=0` (D-03) como propiedades del mapa, no del control de flujo | ✓ Correcto — HCSYNC-01/02/03 cubiertos por helper puro testeable |
| `turnoCtx` pre-fetch extendido (tipoTurnoId + tipoTurno.nombre) fuera de la transacción — v1.15 | Patrón pgBouncer ya estándar desde v1.6: evitar un segundo query dentro de la `$transaction` que pueda colgar el pool | ⚠️ Revisit — deuda WR-01: deja una ventana TOCTOU angosta (hoy inalcanzable en prod, sin path de borrado de turno); hardening opcional vía `tx.turno.updateMany` |
| Guard endurecido a `if (dto.turnoId && turnoCtx)` en el sync de `crearEntrada` — v1.15 | Un `turnoId` stale o borrado hacía que `tx.turno.update` corriera contra una fila inexistente (P2025), abortando el guardado completo de la HC — gap confirmada en `65-VERIFICATION.md` | ✓ Correcto — cierra T-65-03; el sync es no-op en vez de romper el guardado |
| Rama de render `pre_quirurgico` en `HCEntryContent.tsx` sin migrar `HistorialClinicoPanel`/`TurnoHCModal` — v1.15 | HCUI-02 sólo necesita el branch faltante; consolidar los tres lectores es un refactor de scope propio | ✓ Correcto — detalle prequirúrgico visible; consolidación sigue diferida (deuda carried desde v1.11) |
