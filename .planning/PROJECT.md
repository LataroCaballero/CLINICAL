...
**Estado:** Phase 62 completa (2026-07-21, 3/3 planes, verificación automática 21/21) — frontend portal + staff: consentimiento con doble gate (abrir-PDF + tildar checkbox, CONS-09/10/12) y sin rastro de indicaciones; sección Indicaciones independiente con acuse on-click (INDIC-01/02); staff ve fecha de lectura en el stepper (INDIC-05) y el board CRM se sincroniza vía refetch on window focus (EMBUDO-06). Milestone v1.14 (Portal — Firma Gated e Indicaciones Separadas) completo (Phases 61+62). Deuda abierta (aceptada tras UAT approved, `62-HUMAN-UAT.md`): CR-03 — `indicacionesLeidasAt` es global/set-once pero el stepper lo muestra por zona (impreciso en pacientes multi-zona); WR-01 — acuse sin manejo de error visible. Próximo: `/gsd:new-milestone`.
...
---
*Last updated: 2026-07-21 after Phase 62 complete (frontend portal + staff v1.14 — milestone Portal: Firma Gated e Indicaciones Separadas completo, Phases 61+62). Deuda aceptada: CR-03 (stepper per-zone vs campo global) y WR-01 (acuse sin onError).*
