...
<task type="auto">
  <name>Task 2: Public controller + nuevos endpoints en PresupuestosController + module wiring</name>
  <files>
    backend/src/modules/presupuestos/presupuesto-public.controller.ts
    backend/src/modules/presupuestos/presupuestos.controller.ts
    backend/src/modules/presupuestos/presupuestos.service.ts
...
También agregar al constructor de `PresupuestosService` la inyección de `PresupuestoPdfService`:
```typescript
constructor(
  private readonly prisma: PrismaService,
...
```
Build sin errores. Luego verificar que los endpoints existan:
```bash
...
```
  ...
  <done>
`npm run build` sin errores TypeScript. El endpoint `GET /presupuestos/public/:token` retorna 404 (no 401) para tokens inexistentes — confirma que no requiere autenticación. El endpoint `GET /presupuestos/:id/pdf` existe y tiene el decorador `@Auth()` correcto. `presupuestos.module.ts` registra los 4 servicios y 2 controladores.
  </done>
</task>
