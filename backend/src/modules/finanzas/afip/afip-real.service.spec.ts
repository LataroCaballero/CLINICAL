...
function buildMockPrisma(overrides: Record<string, any> = {}) {
  ...
  const mockAfipConfigFindUniqueOrThrow = jest
    .fn()
    .mockResolvedValue(mockConfig);
  const mockFacturaFindUniqueOrThrow = jest
    .fn()
    .mockResolvedValue(mockFacturaFields);
  ...
  const mockTransaction = jest
    .fn()
    .mockImplementation(async (fn: (tx: any) => Promise<any>) => fn(mockTx));
...
describe('AfipRealService', () => {
  ...
  it('throws AfipBusinessError with Spanish message when resultado=R', async () => {
    ...
    await expect(service.emitirComprobante(buildBaseParams())).rejects.toThrow(
      AfipBusinessError,
    );
  });
  ...
  it('QR-01: factura.update is called with qrData starting with AFIP QR base URL', async () => {
    ...
    expect(updateCall.data.qrData).toMatch(
