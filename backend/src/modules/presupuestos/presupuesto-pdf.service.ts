...
export class PresupuestoPdfService {
  ...
  private async buildHeader(doc: PDFKit.PDFDocument, data: PresupuestoPdfData) {
    ...
    if (config.logoUrl) {
      try {
        const resp = await axios.get(config.logoUrl, {
    ...
    doc.text(`Nro: ${data.id.slice(0, 8).toUpperCase()}`, 400, 64, {
      align: 'right',
      width: 145,
    });
    if (data.fechaValidez) {
      const validezStr = new Date(data.fechaValidez).toLocaleDateString(
        'es-AR',
      );
      doc.text(`Válido hasta: ${validezStr}`, 400, 78, {
        align: 'right',
        width: 145,
      });
    }
  ...
  private buildPatientSection(
    doc: PDFKit.PDFDocument,
    paciente: PresupuestoPdfData['paciente'],
  ) {
  ...
  private buildItemsTable(
    doc: PDFKit.PDFDocument,
    items: PresupuestoPdfData['items'],
    moneda: string,
  ) {
  ...
  private buildTotals(doc: PDFKit.PDFDocument, data: PresupuestoPdfData) {
    ...
    doc.text(`${simbolo} ${data.subtotal.toLocaleString('es-AR')}`, 450, y, {
      align: 'right',
      width: 95,
    });
    y += 18;
    ...
    if (data.descuentos > 0) {
      ...
      doc.text(
        `- ${simbolo} ${data.descuentos.toLocaleString('es-AR')}`,
        450,
        y,
        { align: 'right', width: 95 },
      );
    ...
    doc.text(`${simbolo} ${data.total.toLocaleString('es-AR')}`, 450, y, {
      align: 'right',
      width: 95,
    });
  ...
  private buildFooter(
    doc: PDFKit.PDFDocument,
    config: PresupuestoPdfData['config'],
    notaCoordinador?: string,
  ) {
